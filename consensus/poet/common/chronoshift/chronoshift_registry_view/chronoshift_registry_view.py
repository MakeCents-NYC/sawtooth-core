...
class ChronoShiftRegistryView(object):
    """
    ...
    def get_validators(self):
        ...
        """
        validator_map_addr = ChronoShiftRegistryView._to_address('validator_map')
        leaves = self._state_view.leaves(_NAMESPACE)
        infos = [
            ChronoShiftRegistryView._parse_validator_info(state_data)
    ...
    def has_validator_info(self, validator_id):
        ...
        try:
            self._state_view.get(
                ChronoShiftRegistryView._to_address(validator_id))
            return True
        except KeyError:
            return False
    ...
    def get_validator_info(self, validator_id):
        ...
        return ChronoShiftRegistryView._parse_validator_info(state_data)

    @staticmethod
    def _to_address(addressable_key):
        return _NAMESPACE + hashlib.sha256(
            addressable_key.encode('utf-8')).hexdigest()
