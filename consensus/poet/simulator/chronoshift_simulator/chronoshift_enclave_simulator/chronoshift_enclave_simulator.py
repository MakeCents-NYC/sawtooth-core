...
from chronoshift_simulator.chronoshift_enclave_simulator.utils import dict2json
from chronoshift_simulator.chronoshift_enclave_simulator.utils import json2dict

from chronoshift_simulator.chronoshift_enclave_simulator.enclave_signup_info \
     import EnclaveSignupInfo
from chronoshift_simulator.chronoshift_enclave_simulator.enclave_wait_timer \
     import EnclaveWaitTimer
from chronoshift_simulator.chronoshift_enclave_simulator.enclave_wait_certificate \
     import EnclaveWaitCertificate
...
class _ChronoShiftEnclaveSimulator(object):
    ...
    def create_wait_timer(cls,
        ...
        with cls._lock:
            ...
            duration = \
                _ChronoShiftEnclaveSimulator.MINIMUM_WAIT_TIME \
...
def initialize(config_dir, data_dir):
    _ChronoShiftEnclaveSimulator.initialize(config_dir=config_dir, data_dir=data_dir)


def shutdown():
    _ChronoShiftEnclaveSimulator.shutdown()


def get_enclave_measurement():
    return _ChronoShiftEnclaveSimulator.get_enclave_measurement()


def get_enclave_basename():
    return _ChronoShiftEnclaveSimulator.get_enclave_basename()


def create_signup_info(originator_public_key_hash,
                       nonce):
    return \
        _ChronoShiftEnclaveSimulator.create_signup_info(
            originator_public_key_hash=originator_public_key_hash,
            nonce=nonce)


def deserialize_signup_info(serialized_signup_info):
    return _ChronoShiftEnclaveSimulator.deserialize_signup_info(
        serialized_signup_info=serialized_signup_info)


def unseal_signup_data(sealed_signup_data):
    return _ChronoShiftEnclaveSimulator.unseal_signup_data(sealed_signup_data)


def release_signup_data(sealed_signup_data):
    return _ChronoShiftEnclaveSimulator.release_signup_data(sealed_signup_data)
...
def create_wait_timer(sealed_signup_data,
                      ...
                      local_mean):
    return \
        _ChronoShiftEnclaveSimulator.create_wait_timer(
            sealed_signup_data=sealed_signup_data,
            validator_address=validator_address,
            previous_certificate_id=previous_certificate_id,
            local_mean=local_mean)
...
def deserialize_wait_timer(serialized_timer, signature):
    return \
        _ChronoShiftEnclaveSimulator.deserialize_wait_timer(
            serialized_timer=serialized_timer,
            signature=signature)
...
def create_wait_certificate(sealed_signup_data, wait_timer, block_hash):
    return \
        _ChronoShiftEnclaveSimulator.create_wait_certificate(
            sealed_signup_data=sealed_signup_data,
            wait_timer=wait_timer,
            block_hash=block_hash)
...
def deserialize_wait_certificate(serialized_certificate, signature):
    return \
        _ChronoShiftEnclaveSimulator.deserialize_wait_certificate(
            serialized_certificate=serialized_certificate,
            signature=signature)


def verify_wait_certificate(certificate, poet_public_key):
    _ChronoShiftEnclaveSimulator.verify_wait_certificate(
