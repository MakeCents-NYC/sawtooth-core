...
from stake.protobuf.stake_payload_pb2 import MintStakeTransactionData
from stake.protobuf.stake_payload_pb2 import SendStakeTransactionData
from stake.protobuf.stake_payload_pb2 import LockStakeTransactionData
...
class StakeTransactionHandler(TransactionHandler):
    ...
    def apply(self, transaction, context):
        ...
        if id_type == StakePayload.SEND_STAKE:
            send=payload.send
            _apply_send(send,context,signer)
...
def _apply_send(send_data, context, public_key):
    ...
    receiver_address = _stake_to_address(send_data.toPubKey)
    ...
    receiver_stake = receiver_stake_list.stakeMap.get_or_create(send_data.toPubKey)

    if receiver_stake.blockNumber > current_block:
        raise InvalidTransaction("The stake at {} is locked".format(send_data.toPubKey))
    # business logic
    if send_data.value is 0:
        raise InvalidTransaction("Zero amount transactions are forbidden")
    if send_data.value > sender_stake.value:
        raise InvalidTransaction("Insufficient Balance")
    receiver_stake = _get_stake(send_data.toPubKey, context)
    senderstake_list = get_stakelist(sender_stake.ownerPubKey, context)
    receiverstake_list = get_stakelist(send_data.toPubKey, context)
    if receiver_stake is None:
        receiver_stake = _create_stake(send_data, current_block)
    else:
        # if locked(sender_stake, current_block) or locked(receiver_stake):
        #     raise InvalidTransaction("Transaction stake is locked")
        # else:
        sender_stake = _update_stake(sender_stake, 'sender', send_data.value)
        receiver_stake = _update_stake(receiver_stake, 'receiver', send_data.value)
