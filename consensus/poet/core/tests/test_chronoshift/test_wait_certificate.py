...
import chronoshift_simulator.chronoshift_enclave_simulator.chronoshift_enclave_simulator \
    as chronoshift_enclave
...
from  chronoshift.protobuf.chronoshift_registry_pb2 \
    import CSignUpInfo
from test_chronoshift.utils import create_random_public_key_hash
...
class TestWaitCertificate(TestCase):
    ...
    def test_create_after_wait_timer_timed_out(self):
        ...
        while not wt.has_expired(time.time()):
            time.sleep(1)
        time.sleep(self.chronoshift_enclave_module.TIMER_TIMEOUT_PERIOD + 1)
    ...
    def test_create(self):
        ...
        wt = \
            WaitTimer.create_wait_timer(
                chronoshift_enclave_module=mock_poet_enclave_module,
                sealed_signup_data=signup_info.sealed_signup_data,
                validator_address='1660 Pennsylvania Avenue NW',
                previous_certificate_id=NULL_BLOCK_IDENTIFIER,
                consensus_state=self.mock_consensus_state,
