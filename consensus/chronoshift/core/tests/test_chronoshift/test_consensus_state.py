...
from chronoshift_cs.chronoshift_consensus import consensus_state
...
class TestConsensusState(TestCase):
    ...
    def test_serialize(self):
        ...
        with mock.patch(
                'chronoshift_cs.chronoshift_consensus.consensus_state.cbor.loads') \
                as mock_loads:
        ...
        for invalid_alm in [None, 'not a float', (), [], {}, -1,
            ...
            state = consensus_state.ConsensusState()
            with mock.patch(
                    'chronoshift_cs.chronoshift_consensus.consensus_state.cbor.'
                    'loads') \
                    as mock_loads:
                mock_loads.return_value = {
    ...
    def test_local_mean(self):
        ...
        def _compute_fixed_local_mean(count):
            ratio = \
                1.0 * count / \
                mock_chronoshift_settings_view.population_estimate_sample_size
            return \
                (mock_chronoshift_settings_view.target_wait_time * (1 - ratio**2)) + \
                (mock_chronoshift_settings_view.initial_wait_time * ratio**2)
        ...
        wait_certificates = []
        state = consensus_state.ConsensusState()
        sample_size = mock_chronoshift_settings_view.population_estimate_sample_size
        for _ in range(sample_size):
            ...
            self.assertAlmostEqual(
                ...
                second=state.compute_local_mean(mock_chronoshift_settings_view),
                places=4)
        ...
        def _compute_historical_local_mean(wcs):
            ...
            return mock_chronoshift_settings_view.target_wait_time * (sm / sw)
    ...
    def test_block_claim_limit(self):
        ...
        # "claim" blocks
        for _ in range(mock_chronoshift_settings_view.key_block_claim_limit):
            self.assertFalse(
    ...
    def test_block_claim_delay(self):
        ...
        mock_chronoshift_settings_view = mock.Mock()
        mock_chronoshift_settings_view.key_block_claim_limit = 10000
        mock_chronoshift_settings_view.block_claim_delay = 2
        mock_chronoshift_settings_view.population_estimate_sample_size = 50
    ...
    def test_block_claim_frequency(self, mock_deserialize):
        ...
        """
        mock_chronoshift_settings_view = mock.Mock()
        mock_chronoshift_settings_view.target_wait_time = 5.0
        mock_chronoshift_settings_view.key_block_claim_limit = 10
        mock_chronoshift_settings_view.population_estimate_sample_size = 50
        ...
        mock_wait_certificate.duration = 3.14
        mock_wait_certificate.local_mean = \
            mock_chronoshift_settings_view.target_wait_time * 2
        ...
        observed = mock_chronoshift_settings_view.ztest_minimum_win_count
        while True:
            expected = \
                float(observed) / \
            ...
            if z_score > mock_chronoshift_settings_view.ztest_maximum_win_deviation:
                break
    ...
    def test_signup_commit_maximum_delay(self):
        ...
        mock_chronoshift_settings_view = mock.Mock()
        mock_chronoshift_settings_view.signup_commit_maximum_delay = 1
        ...
        with mock.patch('chronoshift_cs.chronoshift_consensus.consensus_state.utils.'
        ...
        with mock.patch('chronoshift_cs.chronoshift_consensus.consensus_state.utils.'
            ...
            for delay in range(len(block_dictionary) - 1):
                mock_chronoshift_settings_view.signup_commit_maximum_delay = delay
                self.assertTrue(
                    state.validator_signup_was_committed_too_late(
                        validator_info=validator_info,
                        chronoshift_settings_view=mock_chronoshift_settings_view,
    ...
    def test_stake_amount_is_low(self):
        """Verify that consensus state properly indicates whether or not a
        validator has reached the block claim limit
        """
        mock_chronoshift_settings_view = mock.Mock()
