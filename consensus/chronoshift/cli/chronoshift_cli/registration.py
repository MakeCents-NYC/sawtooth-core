...
from chronoshift_cli import config
from chronoshift_cli.exceptions import CliException
from chronoshift_cli.chronoshift_enclave_module_wrapper import \
    ChronoShiftEnclaveModuleWrapper
...
from chronoshift_cs.chronoshift_consensus.chronoshift_stake_view \
    import StakeView
import chronoshift.protobuf.chronoshift_registry_pb2 as cr_pb
...
def do_create(args):
    ...
    stake_address= StakeView.stake_address(public_key)
    with ChronoShiftEnclaveModuleWrapper(
            enclave_module=args.enclave_module,
            config_dir=config.get_config_dir(),
            data_dir=config.get_data_dir()) as chronoshift_enclave_module:
    ...
    payload = \
        cr_pb.ChronoShiftRegistryPayload(
            verb='register',
            name='validator-{}'.format(public_key[:8]),
            id=public_key,
            signup_info=cr_pb.CSignUpInfo(
